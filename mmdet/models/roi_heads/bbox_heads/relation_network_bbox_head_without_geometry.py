...
class ObjectRelationModuleWithoutGeometry(nn.Module):
    ...
    def init_weight(self):
        for m in [self.q_fc, self.k_fc, self.v_fc, self.y_fc]:
    ...
    def forward(self, x, position_embedding):
        ...
        weight_all = torch.softmax(weight, dim=2)
...
class RelationNetworkBBoxHeadWithoutGeometry(BBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(RelationNetworkBBoxHeadWithoutGeometry, self).__init__(*args, **kwargs)
        ...
        self.rm1 = nn.ModuleList([
            ObjectRelationModuleWithoutGeometry(df=fc1_dim, dk=dk, dg=dg, Nr=Nr) for _ in range(r1)
        ])
        self.rm2 = nn.ModuleList([
            ObjectRelationModuleWithoutGeometry(df=fc2_dim, dk=dk, dg=dg, Nr=Nr) for _ in range(r2)
        ])
    ...
    def init_weights(self):
        super(RelationNetworkBBoxHeadWithoutGeometry, self).init_weights()
        ...
        for module in self.rm1:
            if isinstance(module, ObjectRelationModuleWithoutGeometry):
                module.init_weight()
